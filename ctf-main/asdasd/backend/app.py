...
def floor_time(dt: datetime, minutes: int = 5) -> datetime:
    discard = timedelta(minutes=dt.minute % minutes, seconds=dt.second, microseconds=dt.microsecond)
    return dt - discard


def ceil_time(dt: datetime, minutes: int = 5) -> datetime:
    floored = floor_time(dt, minutes)
    if floored == dt:
        return dt
...
def api_activity():
    ...
    for sub, u, c in q:
        out.append({
            ...
            "title": c.title,
            "challenge": c.title,
            "time": sub.created_at.strftime("%Y-%m-%d %H:%M:%S"),
        })
    return jsonify(out)
...
def api_scoreboard_pro():
    ...
    solve_rows = (
        db.session.query(Solve.user_id, Solve.challenge_id, Solve.created_at, Challenge.points, Challenge.category)
        .join(Challenge, Challenge.id == Solve.challenge_id)
        .filter(Solve.user_id.in_(user_ids) if user_ids else False)
        .all()
    )
    ...
    challenge_solves = defaultdict(list)
    solved_map = defaultdict(set)
    for uid, cid, solved_at, points, category in solve_rows:
        challenge_solves[cid].append((solved_at, uid))
        solved_map[uid].add(cid)
        user = stats.get(uid)
        if not user:
            continue
        user["score"] += int(points or 0)
        user["solve_count"] += 1
        user["categories"][category] += 1
        user["events"].append((solved_at, int(points or 0)))
        if user["last_update"] is None or solved_at > user["last_update"]:
            user["last_update"] = solved_at

    award_map = defaultdict(dict)
    first_bloods = defaultdict(int)
    for cid, entries in challenge_solves.items():
        entries.sort(key=lambda x: x[0])
        for idx, (_, uid) in enumerate(entries[:3], start=1):
            award_map[cid][uid] = idx
            if idx == 1:
                first_bloods[uid] += 1

    rows = []
    for u in users:
        user_stat = stats[u.id]
        categories = user_stat["categories"]
        strongest_cat = max(categories, key=categories.get) if categories else "-"
        rows.append({
            "user_id": u.id,
            "username": u.username,
            "score": user_stat["score"],
            "solve_count": user_stat["solve_count"],
            "solved": user_stat["solve_count"],
            "strongest_cat": strongest_cat,
            "first_bloods": first_bloods.get(u.id, 0),
            "last_update": user_stat["last_update"].strftime("%Y-%m-%d %H:%M:%S") if user_stat["last_update"] else "-",
        })

    rows.sort(key=lambda x: (-x["score"], -x["solve_count"], x["username"]))
    ...
    if solve_rows:
        ...
        earliest = now
        latest = now
    start_time = floor_time(earliest)
    end_time = ceil_time(max(now, latest))
    ...
    trends = {}
    for u in users:
        events = sorted(stats[u.id]["events"], key=lambda x: x[0])
        points = []
        total = 0
        idx = 0
        for label_time in label_times:
            while idx < len(events) and events[idx][0] <= label_time:
                total += events[idx][1]
                idx += 1
            points.append(total)
        trends[u.username] = points
    ...
    for row in rows:
        ...
        for challenge in challenges:
            status = 0
            if award_map.get(challenge.id, {}).get(user_stat["user_id"]):
                status = award_map[challenge.id][user_stat["user_id"]]
            elif challenge.id in solved_map.get(user_stat["user_id"], set()):
                status = 4
            matrix[challenge.id] = status
        matrix_users.append({
            **{k: v for k, v in user_stat.items() if k != "user_id"},
            "matrix": matrix,
        })

    payload = {
        "users": matrix_users[:100],
        "challenges": [{"id": c.id, "title": c.title, "points": c.points} for c in challenges],
        "trends": trends,
        "trend_labels": labels,
    }
    return jsonify({"data": payload, **payload})
...
@app.route("/api/admin/download_audit", methods=["GET"])
@jwt_required()
@admin_required
def api_admin_download_audit(admin_user):
    solves = (
    ...
    downloads = {
        (d.user_id, d.challenge_id)
        for d in DownloadLog.query.all()
    }
    out = []
    for s, u, c in solves:
        downloaded = (u.id, c.id) in downloads
        out.append({
            "username": u.username,
            "real_name": u.real_name,
            "challenge": c.title,
            "category": c.category,
            "solved_at": s.created_at.strftime("%Y-%m-%d %H:%M:%S"),
            "downloaded": downloaded,
            "status": "已下载附件" if downloaded else "未下载附件但已解题",
        })
    return json_response(data=out)
